...
Allows VSCodium transparency. Confirmed working as of version **1.106.38432**
...
4a. Pro tip: If you need to compile a specific version, use `RELEASE_VERSION=1.106.38432 ./build/build.sh`. This is the version I tested these patches with. Use a version listed in vscodium's "Releases" page on GitHub for compatibility with [Open-SSH-Remote](https://github.com/jeanp413/open-remote-ssh/).
...
## Updating patches (If build says patches failed/corrupt patch files)
...
4. Find the relevant area modified by the patch. If the patch failed it's likely code was moved around a bit in an update to vscode.
...
7. Run `git diff > ../transparency.patch`, rename if needed
